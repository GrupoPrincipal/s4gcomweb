...
var loadProductList = function(type){   
    ...
    $.ajax({
        ...
        for (i = 0; i < response.length; i = i + 1) {
            ...
            tag = '<div class="card simpleCart_shelfItem" id="card' + i + '" data-id="' + response[i].ARTV_IDARTICULO + '" data-uxc="' + response[i].ARTN_UNIXCAJA + '" data-iva="' + response[i].ARTN_PORCIVA + '" data-pre="' + response[i].ARTN_PRECIOCAJ + '" data-des="' + 0 + '" data-rel="' + response[i].PROV_IDPROMO + '" data-por="' + response[i].PRON_DESC1 + '" data-esc="' + response[i].PRON_DESDE1 + '" data-min="'+response[i].ARTN_UNDMIN+'">' +
                  '  <div class="card-image">'+
                  '    <img class="item_image-' + response[i].ARTV_IDARTICULO + '" src="./images/no-image.png" />' +
                  '  </div>'+
                  '  <h5 class="item_name">' + sDescrip.substr(0,30) + '</h5>' +
                  '  <h5 class="item_name">' +  (sDescrip.substr(31,60).trim()  === "" ? "~" : sDescrip.substr(31,60)) + '</h5>' +
                ...
                if(_tmp.tipo == 1){
                  tag+=  '      <h5 class = "'+ (!xPro ? "normal" : "tachado") + '">EXIST CJS. ' + (response[i].EXDEV_UNIDADES/response[i].EXDEN_UNIXCAJA).toFixed(2) + '</h5>';                    
                }
            tag+=  '      <h5 class = "'+ (!xPro ? "normal" : "tachado") + '">DESC MIN: ' + response[i].ARTV_DESCUNIMIN + '</h5>';
             tag+='     </div>' +
                  ...
                  '  </div>' +
                  '  <div class="item_qnt_controls" style="background-color: #146eb4">' +
